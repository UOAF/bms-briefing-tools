...
- State phased movement as explicit conditions: what must be destroyed or confirmed before each element advances, which objectives are mandatory, and which follow-ons are optional.
- Use stable named tactical points in execution prose. Do not describe the common plan with one flight's steerpoint numbers; keep those indices in the flight-plan appendix.
...
- Old mission context.
- Duplicate aliases for one physical site, such as both `SA5` and `5`, duplicate `11`, or a prior-event SAM name alongside its current mission name.
...
- Per-aircraft loadout summarized by useful weapons, sensors, and tanks; label the package-table column `Loadout / A/C` so the counts are not mistaken for flight totals.
- Reject a player package table containing `not listed` loadouts. Empty player stores require investigation of the decoded flight record and pyopencam adapter, not silent omission.
- Laser codes where decoded/applicable.
- A-A TACAN channels per ship for every player flight, generated by the deterministic package-block scheme and formatted compactly, for example `15X / 78X / 78Y / 15Y` for ships #1-#4. Never leave a player flight as `not assigned` merely because the context lacks an explicitly expanded list.
...
The weather map is a meteorological decision aid, not a second steerpoint spaghetti chart. For a multi-flight or multi-package operation, reuse the route overview's mission-context `map_flow_groups` as the default player-route layer, but shorten their weather-map labels to role names such as `SEAD`, `CORRIDOR`, and `EAST`; callsign rosters belong elsewhere. Only draw every individual flight plan when consolidated flow groups are unavailable or deliberately disabled. Always overlay the decoded AWACS and tanker routes, deduplicate support assets linked by more than one player package, and keep their callsign/role labels readable. Preserve the established primary-package player-route crop: neither additional packages nor support tracks may expand it and unexpectedly zoom the map out; clip their geometry at the established edges.

Render the player-route layer, including its labels and label backgrounds, at 20% opacity by default. Weather cells, weather samples, AWACS/tanker tracks, scale, and other meteorological annotations remain on separate layers and must not be faded with it.
